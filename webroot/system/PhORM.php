...
class PhORM extends PholdBoxBaseObj
{
    ...
    function __construct()
    {
        ...
        try {
            $connectionString = $this->SYSTEM["dsn"][$current_dsn]["connection_string"];
     ...
     * getValue
     * 
     * Gets value of key from the value object
     *
     * @param [type] $key
     * @return void
     */
     ...
     * __call
     * 
     * This is a php 5 "magic" function that gets called if you call a function that doesn't exist.
     ...
     * getPhormTable
     *
     * returns the table this object is associated with.
     * @return String
     */
     ...
     * generateSelect
     * 
     * Generates a select statement based on the values in the DB object.  The where clause is
     * generated by ANDing the values of the object together.
     *
     * Returns exectued PDO statement.
     * 
     * @param boolean $fts
     * @return void Executed PDO staement
     */
    ...
    public function load($fts = false)
    {
        ...
        if ($result->rowCount() == 1) {
            $row = $result->fetch();
            foreach ($this->ORM["columns"] as $column) {
                $this->setValue($column, $row[$column]);
            }
        ...
        //load objects
            ...
            while ($row != null) {
                $obj = new $class;
                foreach ($this->ORM["columns"] as $column) {
                    $obj->setValue($column, $row[$column]);
                }
     ...
     *
     * @return void Executed PDO statement
     */
     ...
     *
     * @return void Exetued PDO statement
     */
     ...
     * generateBulkInsert
     * 
     * generates the bulk insert sql
     *
     * @param [type] $tempTableKey
    ...
    public function bulkSave($items)
    {
        ...
        if ($insertCount != 0) {
            $insertSQL .= ";";
            $result["insert"] = $this->db->query($insertSQL);
            // Always check that result is not an error
            if ($this->db->errorCode() != "00000") {
                throw new \Exception($this->db->errorInfo()[2]);
            }
        ...
        if ($updateCount != 0) {
            ...
            foreach ($updateSQLArray as $sql) {
                $result["update"] = $this->db->query($sql);
            }
